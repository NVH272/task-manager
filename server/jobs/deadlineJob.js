...
// Hàm tạo thông báo trong app (In-app notification)
const createInAppNotification = async (userId, taskId, title, timeMessage) => {
    await Notification.create({
        user: userId,
        task: taskId,
        message: `Công việc "${title}" ${timeMessage}.`
    });
};
        ...
        try {
            ...
            for (let task of tasks) {
                ...
                if (diff <= ONE_DAY && diff > ONE_HOUR && !task.notifiedAt.oneDay) {
                    ...
                    await createInAppNotification(task.user._id, task._id, task.title, "sẽ hết hạn trong 1 ngày nữa");
                ...
                // 2. Còn 1 giờ (Khoảng từ 30m đến 60m)
                    ...
                    await createInAppNotification(task.user._id, task._id, task.title, "sẽ hết hạn trong 1 giờ nữa");
                ...
                // 3. Còn 30 phút (Khoảng từ 5m đến 30m)
                    ...
                    await createInAppNotification(task.user._id, task._id, task.title, "sẽ hết hạn trong 30 phút nữa");
                ...
                // 4. Còn 5 phút (Khoảng từ 0m đến 5m)
                    ...
                    await createInAppNotification(task.user._id, task._id, task.title, "RẤT GẤP: Sẽ hết hạn trong 5 phút nữa");
                ...
                // 5. Quá hạn 10 phút (Khoảng từ -10m đến -11m)
                    ...
                    await createInAppNotification(task.user._id, task._id, task.title, "ĐÃ QUÁ HẠN 10 PHÚT");
